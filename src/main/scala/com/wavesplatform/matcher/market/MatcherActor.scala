...
class MatcherActor(recoveryCompletedWithCommandNr: Long => Unit,
  ...
  private def forwardToOrderBook: Receive = {
    ...
    case x: Request.Place  => runFor(x.payload.assetPair)((_, orderBook) => orderBook ! x.payload)
    case x: Request.Cancel => runFor(x.inner.assetPair)((_, orderBook) => orderBook ! x.inner)
  ...
  override def receiveRecover: Receive = {
    ...
    case RecoveryCompleted =>
      ...
      if (orderBooks.get().nonEmpty) context.become(collectOrderBooks(orderBooks.get().size, Long.MaxValue))
  }
  ...
  object Request {
    ...
    def toBytes(x: Request): Array[Byte] = Longs.toByteArray(x.seqNr) ++ {
      x match {
        ...
        case x: Place           => (2: Byte) +: x.payload.bytes()
