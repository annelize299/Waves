...
class AddressActor(validator: OrderValidator, matcherActor: ActorRef) extends Actor {
    ...
    case CancelOrder(id) =>
      activeOrders.get(id) match {
        case Some(lo) =>
          activeOrders -= id
          release(lo)
          matcherActor.forward(Events.OrderCanceled(lo, false))
