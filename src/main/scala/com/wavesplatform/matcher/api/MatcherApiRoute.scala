...
case class MatcherApiRoute(assetPairBuilder: AssetPairBuilder,
                           ...
                           addressActor: Address => Option[ActorRef],
  ...
  private def withAddress(address: Address): Directive1[ActorRef] = addressActor(address) match {
    case None      => complete(StatusCodes.InternalServerError -> s"Error processing request from $address")
    case Some(ref) => provide(ref)
  }

  private def askAddressActor(address: Address, message: Any): Route =
    complete(addressActor(address) match {
      case Some(ref) => (ref ? message).mapTo[MatcherResponse]
      case None =>
        log.warn(s"Error sending $message to $address")
        Future.successful(StatusCodes.InternalServerError)
    })
  ...
  def place: Route = (path("orderbook") & pathEndOrSingleSlash) {
    post {
      entity(as[Order]) { order =>
        withAddress(order.sender) { ref =>
          complete(placeTimer.measureFuture((ref ? order).mapTo[MatcherResponse]))
        }
      }
    } ~ complete(StatusCodes.MethodNotAllowed)
  }
  ...
  private def handleCancelRequest(assetPair: Option[AssetPair]): Route = {
    withCancelRequest { req =>
      (req.timestamp, req.orderId) match {
        ...
        case _                     => complete(OrderCancelRejected("Either timestamp or orderId must be specified"))
      }
    }
  }
  ...
  def getTradableBalance: Route = (path("orderbook" / AssetPairPM / "tradableBalance" / AddressPM) & get) { (pair, address) =>
    withAssetPair(pair, redirectToInverse = true, s"/tradableBalance/$address") { pair =>
      withAddress(address) { ref =>
        complete((ref ? AddressActor.GetTradableBalance(pair)).mapTo[MatcherResponse])
      }
    }
  }
